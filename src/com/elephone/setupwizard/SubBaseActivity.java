...
public abstract class SubBaseActivity extends BaseSetupWizardActivity {
    ...
    protected void onSubactivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if ((resultCode == -1 && requestCode == 1) || (resultCode == 1 && requestCode == 1)) {
            Intent intent = new Intent();
            intent.setComponent(new ComponentName("com.elephanttek.faceunlock",
                        "com.elephanttek.faceunlock.activity.unlock.TipActivity"));
            intent.putExtra("DNK", "DNK");
            startSubactivity(intent, 2);
            return;
        }
