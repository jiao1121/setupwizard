...
import static com.elephone.setupwizard.SetupWizardApp.KEY_BUTTON_BACKLIGHT;
...
public class FinishActivity extends BaseSetupWizardActivity {
    ...
    private static void handlePrivacyGuard(SetupWizardApp setupWizardApp) {
        ...
        if (mPrivacyData != null && mPrivacyData.containsKey(KEY_PRIVACY_GUARD)) {
            ELESettings.Secure.putInt(setupWizardApp.getContentResolver(),
    ...
    private static void writeDisableNavkeysOption(Context context, boolean enabled) {
        ...
        ELESettings.Global.putInt(context.getContentResolver(),
                ELESettings.Global.DEV_FORCE_SHOW_NAVBAR, enabled ? 1 : 0);
/* Think:jiaozhihao on: Wed, 14 Mar 2018 11:07:09 +0800
...
// End of Think:jiaozhihao
        ...
        if (enabled) {
            ELESettings.Secure.putInt(context.getContentResolver(),
                    ELESettings.Secure.BUTTON_BRIGHTNESS, 0);
        } else {
            int currentBrightness = ELESettings.Secure.getInt(context.getContentResolver(),
                    ELESettings.Secure.BUTTON_BRIGHTNESS, 100);
            int oldBright = prefs.getInt(KEY_BUTTON_BACKLIGHT,
                    currentBrightness);
            ELESettings.Secure.putInt(context.getContentResolver(),
                    ELESettings.Secure.BUTTON_BRIGHTNESS, oldBright);
        }
    }
}
